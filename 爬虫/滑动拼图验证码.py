...
verify_style = driver.find_element_by_xpath(
    '/html/body/div/div[2]/div[1]/div[1]').get_attribute('style')   # 获取图形模块样式
verified_style = driver.find_element_by_xpath(
    '/html/body/div/div[2]/div[1]/div[2]').get_attribute('style')  # 获取空缺滑块样式
verified_left = float(re.findall(
    'left:(.*?)px;', verified_style[0]))  # 获取空缺滑块left值
verify_left = float(re.findall(
    'left:(.*?)px;', verify_style[0]))    # 获取图形滑块left值
